// Equivalent to src/routes/personal_projects/dream.ts
// Pure DOM-manipulation helper functions for the Dream animation component.

export function createReflections(ocean_reflection: HTMLElement) {
  const reflections = document.querySelectorAll<HTMLElement>('.to-be-reflected');
  reflections.forEach((el) => {
    const copy = el.cloneNode(false) as HTMLElement;
    const yOffset = Number(el.style.top.split('%')[0]);
    copy.style.top = '';
...
export function moveMoonAndGradient(
  e: MouseEvent | PointerEvent,
  ogMoon: HTMLElement,
  sky: HTMLElement,
  reflectedMoon: HTMLElement,
  ocean_reflection: HTMLElement
) {
  const rect = sky.getBoundingClientRect();
  const xInt = e.clientX - rect.left - innerWidth / 50;
  const yInt = e.clientY - rect.top - innerHeight / 50;
  const x = xInt + 'px';
  const y = yInt + 'px';
  ogMoon.style.top = y;
  ogMoon.style.left = x;
...
export function moveAndShowDragMe(ogMoon: HTMLElement, drag_me_text: HTMLElement) {
  const y = ogMoon.style.top.split('px')[0];
  const yOffset = Number(y) + ogMoon.getBoundingClientRect().height;
  drag_me_text.style.top = yOffset + 'px';
  drag_me_text.style.left = ogMoon.style.left;
  drag_me_text.style.opacity = '1';
}

export function checkAndPerformIfMoonIntersection(
  ogMoon: HTMLElement,
  reflectedMoon: HTMLElement,
  hitBox: HTMLElement
) {
  if (
    hitBox != null &&
    inBounds(ogMoon.getBoundingClientRect(), hitBox.getBoundingClientRect())
  ) {
    const l = hitBox.getBoundingClientRect().left + 'px';
    const t = hitBox.getBoundingClientRect().top + 'px';
...
export function setCircleTextStyle(
  circle: HTMLElement,
  dreamText: string,
  dreamTextSpans: NodeListOf<HTMLElement>
) {
  const radius = circle.offsetWidth / 2;
  dreamTextSpans.forEach((span, i) => {
    const angle = (i / dreamText.length) * 2 * Math.PI;
    ...
    span.style.top = `-23%`;
    span.style.fontSize =
      (radius * 2 * Math.PI) / dreamTextSpans.length + 'px';
