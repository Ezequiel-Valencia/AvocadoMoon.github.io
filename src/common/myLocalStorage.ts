...
function createBooleanStore(key: string, defaultValue = false) {
  if (isLocalStorageItemNull(key)) {
    safeLocalStorage?.setItem(key, String(defaultValue));
  }
  ...
  return {
    ...
    toggle: () =>
      update((v) => {
        safeLocalStorage?.setItem(key, String(!v));
        return !v;
      }),
  };
}
...
function musicPlaybackTime() {
  ...
  const { subscribe, set } = writable(songTimeStamp);
