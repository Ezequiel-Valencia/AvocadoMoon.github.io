<script lang="ts">
  ...
  function whenImageLoaded(imageInfo: any) {
    ...
    decoderWorker.onmessage = async (e: MessageEvent<StenographyWorkerPayload>) => {
      ...
      message = isAlphaNumeric(decodedMessage)
        ? decodedMessage
...
<div id="decode-div" style="text-align: center;">
  <h3 class="text" style="">Input Encoded Image</h3>
  ...
  <form style="text-align: center;">
    ...
    <label
      ...
      >Was the message encrypted as well?</label
    >
    ...
    <br />
    <button
      style="width: 15vw; min-width:fit-content; height: 4vh; max-height:max-content; margin-left: auto; margin-top:3vh; text-align:center;"
      on:click={imageFileHasBeenInput}
    >
