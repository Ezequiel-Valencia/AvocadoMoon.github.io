<script lang="ts">
  ...
  async function encode(imageInfo: any) {
    ...
    let encodeMessage: StenographyWorkerPayload = new StenographyWorkerPayload(
      message,
      imageInfo.data,
      imageInfo.width,
      imageInfo.height,
...
<article id="encode-div" style="height: 100%">
  <div id="carousel">
    ...
    <div id="image-and-text">
      ...
      <p
        class="text"
        style="font-size: xx-large; margin-top:0; font-style:italic; padding-bottom:0; margin-bottom:0;"
      >
      ...
      {#if chosenImage == 0}
        <input
          ...
          type="file"
          accept="image/*"
          on:input={imageFileHasBeenInput}
        />
      {/if}
    </div>
  ...
  <form id="input-text" style=" text-align:center;">
    <textarea
      style="width: 60vw; height: 6vh; margin-left:auto; margin-right:auto;"
      id="textArea"
    ...
    <label
      for="checkEncrypt"
      class="text"
      style="margin-left:auto; margin-right:auto; width:fit-content; display:inline-block;"
      >Encrypt Message As Well?</label
    >

    <br />
    <button
      style="width: 15vw; min-width:fit-content; margin-left:auto; margin-right:auto; height: 4vh; max-height:max-content; margin-top: 2vh;"
      on:click={(e) => {
