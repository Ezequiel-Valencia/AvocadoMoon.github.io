  ...
  {#if canAccessServer}
    ...
    <div style="margin: auto; text-align:center;">
      <button
        ...
        >Turn Blacklight {showBlueLight ? 'Off' : 'On'}?</button
      >
    </div>
        ...
        <div class="entry-text {showBlueLight ? 'blacklight' : ''}">
          ...
          {#if entry.latitude != undefined}
            <div>
              ...
              <a
                style="display: inline; text-align:left; padding-right:0px;"
                target="_blank"
                rel="noopener noreferrer"
                href="https://www.google.com/maps?q={entry.latitude},{entry.longitude}"
              >
                <p
                  style="margin-top: 0px; margin-bottom:40px; padding-left:0px; display:inline; text-align:left;"
                >
                  {entry.locationName}
                </p>
              </a>
...
<style lang="scss">
  ...
  .interactive {
    ...
    background: radial-gradient(
        circle at center,
        rgba(46, 119, 255, 0.8) 0,
        rgba(32, 103, 255, 0.63) 50%
      )
      no-repeat;
