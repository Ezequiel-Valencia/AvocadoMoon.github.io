import { safeLocalStorage } from '../../common/myLocalStorage';
...
export function createTransitionControl() {
  ...
  return {
    ...
    updateToTransitioning: () => {
      safeLocalStorage?.setItem(key, transitionStates.transitionToProjects);
      set(transitionStates.transitionToProjects);
    },
    updateToTransitioned: () => {
      safeLocalStorage?.setItem(key, transitionStates.showPersonalProjects);
      set(transitionStates.showPersonalProjects);
    },
  };
}
