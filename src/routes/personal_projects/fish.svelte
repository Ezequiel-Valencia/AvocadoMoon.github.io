...
<div>
  {#each { length: numFishes } as _, i (i)}
    <figure
      class="fish-slide"
      style="--l-val: -{i * (Math.random() * 20) + i * 2}%; 
    ...
    >
      <img
        ...
        alt="fish"
        style="transform: {i % 2 == 0 ? 'scale(-1, 1)' : ''};"
      />
      {#if fishHoverIndex == i}
...
<style lang="scss">
  ...
  .fish:hover {
    ...
    animation:
      shake 0.7s infinite,
      swim-right 40s linear infinite;
    cursor: grab;
  }
  ...
  @keyframes shake {
    ...
    30% {
      transform: translate(13px, 2px) rotate(0deg);
    }
    40% {
